...
useEffect(() => { const handleData = (e) => { const { transactions = [], budget = null, goals = [] } = e.detail || {}; setCategorised(transactions); setBudget(budget); setGoals(goals); };
...
{categorised.length > 0 && <StatsCards transactions={categorised} />}

  {budget && <BudgetBreakdown budget={budget} />}

  {goals.length > 0 && <SmartBudgetGoals goals={goals} />}

  {categorised.length > 0 && <RecentTransactions transactions={categorised} />}
</div>
